...
import { DollarSign, ExternalLink, ChevronDown, ChevronUp, AlertCircle } from "lucide-react";
...
export function FinancePanel({ entries }: Props) {
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const financeEntries = entries
    .filter(
      (e) =>
        e.category === "market" ||
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-muted-foreground ml-1">
          Regulations, compliance & fiscal policy
        </span>
      </div>

      {financeEntries.length === 0 ? (
        <div className="bg-card rounded-lg border border-border card-elevated p-6 text-center">
          <p className="text-sm text-muted-foreground italic">
            No finance-related entries yet. Fetch news to populate this section.
          </p>
        </div>
      ) : (
